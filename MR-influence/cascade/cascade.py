...
def influence_aux(Graph, initial_set, iterations, N):
    sample = np.zeros(N)
    for i in xrange(N):
        Graph_new = independent_cascade(Graph, initial_set, iterations)
        sample[i] = np.sum([Graph_new.node[node]['action'] for node in Graph_new.nodes_iter()])
    return sample


def influence(Graph, initial_set, iterations, N):
	"""
	"""
	return np.mean(influence_aux(Graph, initial_set, iterations, N))
